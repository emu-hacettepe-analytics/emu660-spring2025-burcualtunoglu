...
ggplot(df_gender, aes(x = Year, y = Female_Ratio, group = 1)) +
  ...
  labs(
    title = "Female Share of R&D Personnel in Turkiye (2009–2023)",
...
# 2009–2023 yılları filtrele
enterprises <- enterprises %>% filter(Year >= 2009 & Year <= 2023)
rd <- rd %>% filter(Year >= 2009 & Year <= 2023)

# Eğitim düzeyine göre personel sayılarını özetle
education_summary <- rd %>%
  group_by(Year, `Education level`) %>%
  summarise(Total = sum(Number, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = `Education level`, values_from = Total)

# Girişim verisi ile birleştir
...
for (level in education_levels) {
  ...
  # Sayısal özet
  cat("\n\n========================================\n")
  cat("Regression Summary for:", level, "\n")
  cat("========================================\n")
  summary_out <- summary(model)
  cat("Intercept (β₀):", round(coef(model)[1], 2), "\n")
  cat("Slope (β₁):", round(coef(model)[2], 5), "\n")
  cat("R²:", round(summary_out$r.squared, 4), "\n")
  cat("p-value (slope):", signif(summary_out$coefficients[2, 4], 4), "\n")
  cat("F-statistic:", round(summary_out$fstatistic[1], 2), "\n")
  
  # Grafik
  g <- ggplot(df_regression, aes_string(x = "`Total Number of Enterprises`", y = paste0("`", level, "`"))) +
...
| Variable | Coefficient ((\beta)) | p-value | Significance |
|----|----|----|----|
...
$$
\text{PhD} = 8832 - 0.0036 \cdot \text{Enterprises} + 0.5107 \cdot \text{Master's} + 0.0339 \cdot \text{Bachelor's} - 0.6003 \cdot \text{Associate} + 0.0036 \cdot \text{Other} + 14014 \cdot \text{Female Ratio}
$$
...
| Model   | $R^2$  | Adjusted $R^2$ | AIC      | Description |
|---------|--------|----------------|----------|-------------|
